        ...
        else if(pid == 0)
        {
            ...
            while(isClientwork)
            {
                ...
                else
                {
                    ...
                    if(write(pipe_fd[1], send_buff, sizeof(send_buff)) < 0 )throw errno;
                }
            }
        }
        else
        {
            ...
            while(isClientwork) 
            {
                ...
                for(int i = 0; i < epoll_events_count ;i++)
                {
                    ...
                    {
                        //接受服务端广播消息
                        int ret = recv(sock, recv_buff, sizeof(recv_buff), 0);
                        //清空结构体
                    ...
                    { 
                        //父进程从管道中读取数据
                        int ret = read(events[i].data.fd, recv_buff, sizeof(recv_buff));
                        // ret = 0
                        if(ret == 0)
                        {
                            isClientwork = 0;
